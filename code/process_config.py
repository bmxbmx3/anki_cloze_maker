...
def set_blanks_percent(blanks_per_word_count,per_word_count):
    """
    计算空格率，存入配置文件中。
    ...
    """
    #输入数字前先做判断。
    number_ruler=per_word_count<=0 or blanks_per_word_count<=0 or not str(per_word_count).isdigit() or not str(blanks_per_word_count).isdigit()
    if blanks_per_word_count>per_word_count:
        print("每定长字符数中所含的空格数,不要超过每定长字符数！")
    elif  number_ruler:
        print("请输入正常的数字！即不能是非正数或小数。")
    else:
        blanks_percent=blanks_per_word_count/per_word_count
        cfg = ConfigParser()
        cfg.read(CONFIG_PATH)
        a=cfg.sections()
        ...
        #保存3位小数
        cfg["blanks"]["blanks_percent"]="%.3f" % blanks_percent
        with open(CONFIG_PATH, "w") as conf:
            cfg.write(conf)
        print("成功设置空格相关参数！")
...
def get_valid_blanks_count(text):
    ...
    cfg = ConfigParser()
    cfg.read(CONFIG_PATH)
    blanks_percent=float(cfg["blanks"]["blanks_percent"])
    blanks=int(blanks_percent*get_valid_char_count(text))
    #防止没有空格。
    if blanks==0:
        blanks+=1
    return blanks
